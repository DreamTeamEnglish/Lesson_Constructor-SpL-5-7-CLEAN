// ============================================================
// CLEAN v24.1.3 · External AI document flow
...
СПЕЦИАЛЬНЫЕ ПРОФИЛИ:
- Across the Curriculum: предметное содержание не декоративно. Для Maths проверяй арифметику, соответствие таблицы/диаграммы/текста и реалистичность способа сбора данных. Если в уроке есть проценты, обеспечь данные, из которых эти проценты действительно получаются.
- Culture Corner / Spotlight on Russia: исходным содержательным источником по умолчанию является текст текущего раздела учебника и записи ученика. Не создавай дополнительные культурные источники, если их не дал конструктор. Итоговый продукт строится на извлечении и переработке подтверждённой информации, а не на «правдоподобных» знаниях модели.
...
Не добавляй текст до [[TECH_MAP_BEGIN]] и после [[HOMEWORK_END]].`;
  ...
  function currentGoldVersion(){
    return window.KA_METHOD_V24?.version || '24.1.3';
  }
    ...
    return `<section class="print-document">
      <div class="print-brand">
        ...
        <div>
          ...
          <span>Методический конструктор · Spotlight 6 · GOLD STANDARD v${currentGoldVersion()}</span>
        </div>
      </div>
      <h1>${labels[key]}</h1>
