...
(function(){
  ...
  function downloadDraft(){
    ...
    a.href=url;
    a.download=`Spotlight_6_${String(lesson?.legacy_id||'lesson').replace(/[^a-z0-9а-я]+/gi,'_')}.lesson`;
  ...
  function applyState(saved){
    ...
    const target=LESSONS.find(x=>x.id===saved.lesson_id);
    if(!target)throw new Error('Урок из файла не найден в золотой базе Spotlight 6.');
    lesson=target;
  ...
  function modal(title,body,actions=''){
    ...
    m.innerHTML=`<section class="v24-dialog"><div class="v24-dialog-head"><div><small>CLEAN v24 · SPOTLIGHT 6</small><h2>${esc(title)}</h2></div><button data-v24-close>×</button></div><div class="v24-dialog-body">${body}${actions?`<div class="v24-dialog-actions">${actions}</div>`:''}</div></section>`;
  ...
  function resourceCards(){
    ...
    const common={
      ...
      P4:[['Коммуникативная ситуация',lesson.micro_situation,'Сохраняйте адресата и понятную цель общения.'],['Итоговый продукт',lesson.product,'Главная задача этапа — получить наблюдаемый речевой продукт.'],['Речевые модели',frames,'Дайте как поддержку, а не как текст для механического чтения.'],['Тематический банк',bank,'Поддержка для слабых учеников и контроля точности.']],
      P5:[['Критерии успеха',success.join(' · '),'Используйте для индивидуальной проверки и конкретной коррекции.'],['Грамматический фокус',lesson.grammar_focus,'Проверяйте форму только вместе со смыслом.'],['Новая мини-ситуация',lesson.micro_situation,'Измените одно условие, чтобы проверить перенос, а не запоминание образца.']],
      P6:[['Can-do цель',canDo,'Верните ученика к цели и попросите доказательство результата.'],['Критерии успеха',success.join(' · '),'Рефлексия должна опираться на конкретный пример работы.'],['Мост к домашнему заданию',lesson.product,'Домашняя работа продолжает продукт урока, а не начинается с новой случайной задачи.']]
    };
  ...
  function tuneLibrary(){
    ...
    lib.querySelector('.v24-full-library').onclick=()=>{
