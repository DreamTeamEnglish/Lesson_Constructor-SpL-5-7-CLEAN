// ============================================================
// CLEAN v24.2.2 · Methodical normalization layer for GOLD 6
...
(function(){
  ...
  function tuneActivity(l,a,newCanDo){
    ...
    if(code==='P3-01'){
      if(idOf(l)==='2a'){
        ...
        out.title='Извлекаем структуру правил Snakes and Ladders';
        out.teacher='Организует чтение реальной инструкции. Просит найти: players, what you need, start, move, ladder, snake, finish.';
        out.students='Находят в тексте оборудование и действия, выстраивают правила в логичном порядке.';
        out.instruction=`Read the Snakes and Ladders instructions. Find: players, what you need, where you start, how you move, what happens on a ladder, what happens on a snake, and how you win.`;
        out.example=`You need the board, a dice and counters. You start on square ONE. A ladder takes you up; a snake takes you down.`;
        out.materials='Spotlight 6, p. 60, Snakes and Ladders instructions; схема Players / Need / Start / Move / Ladder / Snake / Win.';
        out.support='готовые заголовки схемы; слова board, dice, counter, ladder, snake.';
        out.challenge='Кратко пересказать порядок без чтения полного текста.';
        out.criterion='Все элементы взяты из реальной инструкции; порядок действий понятен новому игроку.';
      }else if(idOf(l)==='8e'){
        out.title='Фразы покупателя и кассира';
        out.teacher='Возвращает реальные фразы Ex. 1a и модельный диалог. Просит распределить фразы по ролям и функциям: greeting/help, request, play, seats, choice, payment.';
        out.students='Сортируют реплики по ролям и восстанавливают логическую последовательность бронирования.';
        out.instruction=`Sort the phrases into Customer / Receptionist. Then put the booking steps in order: greeting → play/date → seats → price/choice → payment.`;
        out.example=`Customer: I’d like to book some theatre tickets, please. Receptionist: Which play would you like to see?`;
        out.materials='Spotlight 6, p. 82, Ex. 1a and model dialogue.';
        out.support='две колонки Customer / Receptionist; первые реплики уже распределены.';
        out.challenge='Объяснить, зачем вопрос How would you like to pay? появляется ближе к концу разговора.';
        out.criterion='Реплика соответствует роли и своему месту в логике бронирования.';
      }else if(idOf(l)==='4f'){
    ...
    if(code==='P3-02'){
      if(idOf(l)==='2a'){
        ...
        out.title='Robinson Crusoe: Present Simple в игре';
        out.teacher='Использует реальную Robinson Crusoe board game страницы 60. Просит восстановить формы глаголов, быстро сверить их, затем в группах по четыре сделать короткий игровой проход по заполненному полю, как предлагает учебник.';
        out.students='Заполняют пропуски (sit, play, not feel, cook, miss, grow, explore, not have, eat, go, be, wake, wash) нужной формой; затем сверяют ответы и кратко играют по готовому Robinson Crusoe board game в группе.';
        out.instruction=`Complete the Robinson Crusoe game with the correct verb forms. Compare your answers, then play a short round in a group of four. Say the complete sentence when your counter lands on a square.`;
        ...
        out.support='напоминание: he/she/it + -s; doesn’t + base verb; is for be.';
        out.challenge='Во время короткой игры назвать полное предложение без чтения формы глагола из подсказки.';
        out.criterion='Форма глагола соответствует he и смыслу клетки; ученик может прочитать/произнести полное предложение в ходе игры.';
      }else if(idOf(l)==='8e'){
  ...
  function install(){
    if(window.__KA_METHOD_2422)return true;
    ...
    window.KA_METHOD_V24={version:'24.2.2',productBare,grammarEn,canDo,success,recommendedDefaults};
    window.__KA_METHOD_2422=true;
