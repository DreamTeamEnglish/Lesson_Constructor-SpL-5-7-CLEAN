// ============================================================
// CLEAN v24.2.3 · Methodical normalization layer for GOLD 6
...
(function(){
  ...
  const PRODUCT_EN={
    ...
    "настольная игра Snakes and Ladders о досуге":"Snakes and Ladders game about free-time activities",
  ...
  function repairCoursebookProfiles(){
    for(const l of (window.LESSONS||[])){
      ...
      l.coursebook_core_sequence=r.coursebook_core_sequence||'';
  ...
  function success(l){
    if(idOf(l)==='2a')return[
      'I can find the occasion, date and time in an invitation.',
      'I can ask and answer about a birthday/date.',
      'I use on / in / at correctly for the information I give.',
      'My invitation clearly tells a friend when and where the event is.'
    ];
    if(idOf(l)==='6c')return[
      'I can find what players need and what they do in the Snakes and Ladders instructions.',
      'I can complete the Robinson Crusoe game by choosing the correct verb forms.',
      'My game rules match my board.',
      'Another player can understand how to start and play my game.'
    ];
    if(idOf(l)==='8e')return[
      'I use the customer/receptionist phrases for the correct role.',
      'I give or ask for the play, date and number of seats clearly.',
      'I can ask about price/seat choice/payment and react to the answer.',
      'My partner can complete the booking without guessing missing information.'
    ];
    if(idOf(l)==='4f')return[
  ...
  function tuneActivity(l,a,newCanDo){
    ...
    if(code==='P1-01'){
      ...
      if(idOf(l)==='2a'){
        out.title='Что можно узнать из приглашения?';
        out.teacher='Показывает реальные приглашения A–D из 2a Happy Times. Просит быстро найти occasion, who, date/time и заметить сокращения, не пересказывая текст целиком.';
        out.students='Рассматривают приглашения, называют две реально видимые детали и предполагают, какие языковые средства понадобятся для собственного приглашения.';
        out.instruction=`Look at the invitations A–D. Find two details you can really see: an occasion, a date, a time or a place. What will we learn to do today?`;
        out.example=`I can see a birthday party and a date and time. I think we will learn how to talk about dates and write an invitation.`;
        out.materials='Spotlight 6, pp. 16–17, invitations A–D.';
        out.criterion='Ответ опирается на реальные детали приглашений; тема связана с датой/временем и приглашением.';
      }else if(idOf(l)==='6c'){
        out.title='Как понять правила игры?';
        out.teacher='Открывает 6c Pastimes и показывает Snakes and Ladders. Просит по полю/инструкции назвать, что видят игроки и что, вероятно, нужно делать.';
        out.students='Называют board/dice/counters/squares и прогнозируют, что текст объясняет порядок игры.';
        out.instruction=`Look at Snakes and Ladders. Name two things you can see or read. What information does a new player need?`;
        out.example=`I can see a board and a dice. A new player needs to know where to start and how to move.`;
        out.materials='Spotlight 6, p. 60, Snakes and Ladders text/board.';
        out.criterion='Названы элементы реальной страницы и сформулирована практическая задача: понять правила игры.';
      }else if(idOf(l)==='8e'){
        out.title='Кто это говорит: customer или receptionist?';
        out.teacher='Использует реальные реплики из упр. 1a страницы 82. Просит распределить 2–3 реплики по ролям и определить ситуацию.';
        out.students='Распознают роль говорящего по функции реплики и называют тему — бронирование театральных билетов.';
        out.instruction=`Read the booking phrases. Which are for the receptionist and which are for the customer? What are they talking about?`;
        out.example=`“How can I help you?” is the receptionist. “I’d like to book some theatre tickets, please.” is the customer. They are booking theatre tickets.`;
        out.materials='Spotlight 6, p. 82, Ex. 1a.';
        out.criterion='Роли определены по коммуникативной функции; тема выведена из реальных реплик.';
      }else if(idOf(l)==='4f'){
    ...
    if(code==='P3-01'){
      if(idOf(l)==='2a'){
        out.title='Читаем приглашения как модель';
        out.teacher='Организует поисковое чтение приглашений A–D: occasion, inviter/guest, date/time, place. Затем просит найти сокращения и восстановить их полную форму.';
        out.students='Заполняют компактную таблицу по приглашениям и расшифровывают 2–3 сокращения.';
        out.instruction=`Read the invitations A–D. For each one, find: occasion, who, when and where. Underline two abbreviations and say the full forms.`;
        out.example=`Sat = Saturday. Dec = December. The birthday party is on Saturday 15 December at 17:30.`;
        out.materials='Spotlight 6, p. 16, invitations A–D; таблица Occasion / Who / When / Where.';
        out.support='подсветка строк date/time/place; готовый пример одного приглашения.';
        out.challenge='Преобразовать одну полную дату/день в естественную форму приглашения с сокращением.';
        out.criterion='Ученик извлекает ключевые сведения и понимает типичные сокращения приглашения.';
      }else if(idOf(l)==='6c'){
        out.title='Robinson Crusoe: восстанавливаем действия';
        out.teacher='Сначала сохраняет первое центральное действие страницы 60: Robinson Crusoe board game. Просит предсказать/восстановить формы глаголов о Robinson Crusoe и быстро объяснить одну форму по подлежащему he.';
        out.students='Самостоятельно восстанавливают sit, play, not feel, cook, miss, grow, explore, not have, eat, go, be, wake, wash в нужной форме, затем быстро сверяют ответы в паре.';
        out.instruction=`Complete the Robinson Crusoe game with the correct verb forms. Compare your answers with a partner and explain one form.`;
        out.example=`He sits under the trees. He plays with his dog. He doesn’t feel happy. He cooks on a fire.`;
        out.materials='Spotlight 6, p. 60, Robinson Crusoe board game.';
        out.support='he/she/it + -s; doesn’t + base verb; be → is.';
        out.challenge='Закрыть подсказанный глагол и восстановить полное предложение по смыслу клетки.';
        out.criterion='Форма соответствует he и содержанию клетки; ученик может объяснить хотя бы одну форму.';
      }else if(idOf(l)==='8e'){
        out.title='Фразы ролей и модель Hamlet';
        out.teacher='Сначала возвращает реальные фразы Ex. 1a и распределяет их Customer / Receptionist. Затем ОБЯЗАТЕЛЬНО организует прослушивание и чтение модельного Hamlet booking dialogue страницы 82 и просит извлечь play/date/seats/price/payment/collection/performance time.';
        out.students='Сортируют реплики по ролям, слушают и читают модельный диалог Hamlet, отмечают реальные детали бронирования и восстанавливают функциональную последовательность.';
        out.instruction=`Sort the phrases into Customer / Receptionist. Then listen to and read the Hamlet booking dialogue. Find the play/date, seats, price, payment and collection/performance details. Put the booking steps in order.`;
        out.example=`Receptionist: How can I help you? Customer: I’d like to book some theatre tickets, please. Receptionist: Which play would you like to see? Customer: Hamlet.`;
        out.materials='Spotlight 6, p. 82, Ex. 1a + audio/model Hamlet booking dialogue.';
        out.support='две колонки Customer / Receptionist + карточка функций help / request / play+date / seats / price+choice / payment / collection.';
        out.challenge='Объяснить, почему payment и collection появляются после выбора билетов.';
        out.criterion='Реплики соответствуют ролям; ученик извлекает детали именно из Hamlet model dialogue и понимает порядок бронирования.';
      }else if(idOf(l)==='4f'){
    ...
    if(code==='P3-02'){
      if(idOf(l)==='2a'){
        out.title='Дата, время и день рождения';
        out.teacher='Кратко систематизирует ordinal numbers, months/seasons, telling the time и on/in/at; затем запускает короткий birthday interview по образцу учебника и фиксирует несколько ответов в мини-календаре класса.';
        out.students='Составляют 2 своих примера, задают партнёру When’s your birthday?, фиксируют ответ и добавляют 2–3 даты в мини-календарь класса/пары.';
        out.instruction=`Use on / in / at correctly. Ask your partner: When’s your birthday? Write the answer in the mini birthday calendar. Then make one sentence with a date and one with a time.`;
        out.example=`My birthday is on 5th May. The party is at half past five. My birthday is in spring.`;
        out.materials='Spotlight 6, p. 17; ordinal numbers; months/seasons; clock/time models; prepositions table; mini birthday calendar.';
        out.support='on + day/date; in + month/season; at + exact time.';
        out.challenge='Добавить адрес/место и связать дату и время в одной фразе приглашения.';
        out.criterion='Дата/время понятны; on/in/at выбраны по смыслу; ученик может спросить/ответить о дне рождения и корректно занести дату в мини-календарь.';
      }else if(idOf(l)==='6c'){
        out.title='Читаем правила и играем короткий раунд';
        out.teacher='После заполнения Robinson Crusoe переходит к реальной инструкции Snakes and Ladders: организует listen/read по схеме Players / Need / Start / Move / Ladder / Snake / Win. Затем возвращает группы по четыре к уже заполненному Robinson Crusoe board game для короткого игрового прохода, как требует цепочка страницы.';
        out.students='Находят в инструкции оборудование и порядок действий, кратко восстанавливают правила; затем в группе из четырёх делают несколько ходов по Robinson Crusoe game и при попадании на клетку произносят полное предложение.';
        out.instruction=`Listen to / read the Snakes and Ladders instructions. Find what players need, where they start, how they move, what ladders and snakes do, and how they win. Then play a short Robinson Crusoe round in a group of four and say the complete sentence on your square.`;
        out.example=`You need the board, a dice and counters. Start on square ONE. Roll the dice and move your counter. Go up a ladder and down a snake. The first player to reach the last square wins.`;
        out.materials='Spotlight 6, p. 60: Snakes and Ladders instructions + Robinson Crusoe board game; схема Players / Need / Start / Move / Ladder / Snake / Win; dice/counters при наличии.';
        out.support='заголовки схемы + board / dice / counter / ladder / snake; затем только заголовки.';
        out.challenge='Кратко объяснить правила без чтения текста и произнести игровое предложение без подсказанного глагола.';
        out.criterion='Правила подтверждаются текстом страницы; короткое игровое действие Robinson Crusoe реально состоялось; полные предложения грамматически корректны.';
      }else if(idOf(l)==='8e'){
        out.title='Собираем и разыгрываем бронирование';
        out.teacher='Моделирует сокращённый ход реального диалога: play/date → seats → choice/price → payment. После первого прохода убирает часть опор. В конце использует реальный pronunciation box страницы 82: know/now, coach/couch, tone/town.';
        out.students='Разыгрывают короткий диалог с заменой спектакля/даты/мест, затем повторяют с меньшей опорой; распределяют know/now, coach/couch, tone/town по /əʊ/ и /aʊ/ и повторяют.';
        out.instruction=`Use the booking phrases. Change the play, date and seats. Then repeat with fewer prompts. Finish with: How would you like to pay? / Can I pay by credit card? Then listen and repeat: know / now, coach / couch, tone / town.`;
        out.example=`R: How can I help you? C: I’d like to book two tickets, please. R: Which play would you like to see? C: “The Snow Queen”, on Friday. R: How many seats would you like? C: Two, near the centre, please.`;
        out.materials='Spotlight 6, p. 82, model dialogue + real pronunciation box: know/now, coach/couch, tone/town.';
        out.support='карточка из 5 функций: help / play+date / seats / price+choice / payment.';
        out.challenge='Уточнить ряд/место и способ получения билетов.';
        out.criterion='Реплики соответствуют ролям; бронирование идёт в логичном порядке; ученик различает целевые /əʊ/ и /aʊ/ в словах упражнения.';
      }else if(idOf(l)==='4f'){
    ...
    if(code==='P4-01'){
      if(idOf(l)==='2a'){
        out.title='Главная задача: приглашение на праздник';
        out.mode='индивидуально → пары';
        out.teacher='Предлагает ситуацию «Imagine it is your birthday / school event», напоминает реальные модели приглашений и критерии date + time + place/address. После написания организует короткую взаимопроверку в паре.';
        out.students='Создают короткую invitation card, затем партнёр проверяет, можно ли понять occasion, when and where.';
        out.instruction=`Imagine it is your birthday or a school event. Write a short invitation card. Include the occasion, date, time and place/address. Then exchange cards and check: Do I know what, when and where?`;
        out.example=`You are invited to my birthday party on Sunday, 4th September at 5:45 pm. It is at 17 Green Street. Hope to see you there!`;
        out.materials='Spotlight 6, pp. 16–17; шаблон invitation card; мини-чек-лист What / When / Where.';
        out.support='You are invited to… / on… / at… / My address is… / Hope to see you there!';
        out.challenge='Использовать уместные сокращения даты/дня и добавить короткую дружескую финальную фразу.';
        out.criterion='В приглашении понятны событие, дата, время и место; on/in/at употреблены уместно.';
      }else if(idOf(l)==='6c'){
        out.title='Проект: своя Snakes and Ladders о досуге';
        out.mode='пары / малые группы → короткая взаимопроверка';
        out.teacher='Возвращает к реальной инструкции Snakes and Ladders и проекту на p.60. Выдаёт компактный шаблон поля. Ограничивает продукт, чтобы за 14 минут успеть создать и проверить игру.';
        out.students='Создают короткую Snakes and Ladders о free-time activities: поле, специальные клетки и понятные правила; соседняя пара проверяет, можно ли начать и играть без устного объяснения.';
        out.instruction=`Use the Snakes and Ladders instructions as your model. Make a small Snakes and Ladders game about free-time activities. Write what players need, where they start, how they move, what happens on special squares and how the game ends.`;
        out.example=`FREE-TIME SNAKES & LADDERS. You need a board, a dice and counters. Start on square 1. Roll the dice and move your counter. If you land on “Read a book”, go up. If you land on “Miss your club”, go down. The first player to FINISH wins.`;
        out.materials='Spotlight 6, p. 60; шаблон поля 12–16 клеток; карандаши; при наличии dice/counters.';
        out.support='Need / Start / Move / Special square / Finish — пять обязательных частей инструкции.';
        out.challenge='Добавить 2 специальных клетки с free-time activity и объяснить их действие одной короткой инструкцией.';
        out.criterion='Игра связана с free-time activities; правила соответствуют полю; используются counter/dice и понятная последовательность.';
      }else if(idOf(l)==='8e'){
        out.title='Ролевая ситуация: бронирование театральных билетов';
        out.mode='пары со сменой ролей';
        out.teacher='Сначала опирается на Portfolio poster p.82 (Romeo and Juliet: Fri Sept 27 / Sat Sept 28, 8 pm, £15/£20, 10% student discount) и превращает его в две информационно разные карточки Customer / Receptionist. После первого раунда даёт новый набор данных для переноса. Следит, чтобы ученики действительно спрашивали недостающую информацию.';
        out.students='В первом раунде бронируют билеты по реальному Romeo and Juliet poster страницы 82, выясняя недостающие play/date/seats/price/payment; затем меняются ролями и проводят второй раунд с новыми данными.';
        out.instruction=`Customer: book the tickets and ask for the information you need. Receptionist: ask for the play/date/seats, offer the available seats and price, and ask about payment. Do not show your card.`;
        out.example=`R: How can I help you? C: I’d like to book some theatre tickets, please. R: Which play would you like to see? C: Romeo and Juliet, on Friday, September 27th. R: How many seats would you like? C: Two, please. R: Tickets are £15 or £20, and students get a 10% discount. C: Great. Can I pay by credit card?`;
        out.materials='Spotlight 6, p.82, Portfolio Romeo and Juliet poster; две ролевые карточки с разной информацией; второй набор данных для переноса; чек-лист booking steps.';
        out.support='How can I help you? / I’d like to book… / Which play…? / How many seats…? / How would you like to pay?';
        out.challenge='Уточнить ряд/цену и способ получения билетов; клиент выбирает из двух вариантов мест.';
        out.criterion='Все ключевые данные получены в диалоге; роли не перепутаны; реплики соответствуют реальной задаче бронирования.';
      }else if(idOf(l)==='4f'){
    ...
    if(code==='P5-01'){
      if(idOf(l)==='2a'){
        out.teacher='Даёт новую карточку события с другим occasion/date/time/place. Ученик самостоятельно оформляет короткое приглашение; затем открывается чек-лист.';
        out.students='Самостоятельно пишут invitation card, проверяют What / When / Where и on/in/at; исправляют реальную неточность или отмечают самое сильное место.';
        out.instruction=`Individually write a new invitation card. Include the occasion, date, time and place/address. Do not copy the class invitation.`;
        out.example=`Please come to our school concert on Friday, 12th May at 6 pm. It is at Green School Hall. Hope to see you there!`;
        out.criterion='Приглашение самостоятельное; адресат понимает событие, дату, время и место; языковые формы не мешают смыслу.';
      }else if(idOf(l)==='6c'){
        out.teacher='Выдаёт мини-поле на 6–8 клеток. Ученик самостоятельно пишет краткие правила по структуре Need / Start / Move / Finish и одну специальную клетку; затем сверяет поле и текст.';
        out.students='Создают индивидуальную мини-игру и проверяют, что каждое правило реально соответствует полю.';
        out.instruction=`Individually make a mini Snakes and Ladders game with 6–8 squares. Write what the player needs, how to start and move, one special-square rule and how to win.`;
        out.example=`You need a dice and a counter. Start on square 1. Roll the dice and move. On “Play chess”, go up one square. The first player to FINISH wins.`;
        out.criterion='Мини-игра завершена; counter/dice и порядок действий понятны; правило специальной клетки соответствует полю.';
      }else if(idOf(l)==='8e'){
        out.teacher='Даёт новую карточку бронирования с play/date/seats/price/payment. После самостоятельного письменного диалога показывает чек-лист ролей и логики бронирования.';
        out.students='Самостоятельно пишут короткий диалог Customer / Receptionist, затем проверяют, все ли ключевые данные получены и не перепутаны ли роли.';
        out.instruction=`Individually write a short theatre-ticket booking dialogue for a new situation. Use the real booking functions: help → play/date → seats/price → payment.`;
        out.example=`R: How can I help you? C: I’d like to book two tickets for Saturday, please. R: Which play would you like to see? C: “Oliver!”. R: We have seats near the centre at £18 each. C: That’s fine. Can I pay by credit card?`;
        out.criterion='Диалог самостоятельный; customer/receptionist функции соблюдены; play/date/seats/price/payment логически связаны.';
      }else if(idOf(l)==='4f'){
  ...
  function tuneSpecialHomework(l,homework){
    ...
    if(idOf(l)==='2a'){
      if(out[0]){
        out[0].situation='Сделай короткую invitation card на выбранное событие.';
        out[0].steps=['Выбери occasion.','Укажи date и time.','Добавь place/address.','Перечитай как получатель: понятно ли what / when / where?'];
        out[0].check=['Есть событие','Понятны дата и время','Есть место/адрес','on/in/at использованы уместно'];
      }
      if(out[1])out[1].situation='Создай необычное приглашение (комикс/мини-постер), сохранив обязательные date, time и place.';
      if(out[2])out[2].situation='Подготовь 45–60 секунд: объяви о событии и пригласи одноклассника. Реальное выступление и вопрос будут на следующем уроке.';
      if(out[3]){
        out[3].situation='Подготовь две карточки Birthday Information Gap самостоятельно. На следующем уроке партнёр восстановит даты/время вопросами.';
        out[3].steps=['Сделай полный мини-календарь из 4 событий.','В Card A убери две даты/времени.','В Card B убери другие две.','Сохрани полный ключ для проверки на уроке.'];
      }
      if(out[4]){
        out[4].situation='Сделай 6 matching cards по приглашениям. Дома только подготовь и проверь ключ; игра с партнёром — на следующем уроке.';
        out[4].steps=['Сделай 6 карточек-приглашений с одним пропуском.','Сделай 6 карточек-ответов с датой/временем.','Проверь самостоятельно, что каждая пара однозначна.','Принеси комплект на урок.'];
      }
    }

    if(idOf(l)==='6c'){
      for(const h of out){h.bank='Источник: Spotlight 6 p.60 + собственная игра урока. Ключевые слова: board, dice, counter, square, ladder, snake, roll, move, land, go up/down, win.'}
      if(out[0]){
        out[0].situation='Сделай компактную Snakes and Ladders на 10–12 клеток о free-time activities.';
        out[0].steps=['Нарисуй путь START → FINISH.','Добавь 2 специальные клетки.','Напиши Need / Start / Move / Special / Finish.','Сверь каждое правило с полем.'];
        out[0].check=['Игра связана с досугом','Есть dice и counter','Правила совпадают с полем','Можно играть без устного объяснения'];
      }
      if(out[1])out[1].situation='Создай Pastime Snakes and Ladders с рисунками видов досуга и двумя понятными игровыми действиями.';
      if(out[2])out[2].situation='Подготовь краткое объяснение своей игры по 5 ключевым словам. На следующем уроке партнёр задаст один вопрос.';
      if(out[3])out[3].situation='Подготовь Card A / Card B с разными пропусками в правилах своей игры. Взаимодействие — только на следующем уроке.';
      if(out[4]){
        out[4].steps=['Сделай 6 карточек: equipment, start, move, ladder/snake action, Present Simple Robinson Crusoe, clear-rule check.','На обороте дай ответ/модель.','Проверь, что карточки не повторяют одно и то же действие.'];
        out[4].check=['Ровно 6 карточек','Есть и game rules, и Present Simple','Используется counter, а не случайная лексика другого урока','Ответы однозначны'];
      }
    }

    if(idOf(l)==='8e'){
      for(const h of out){h.bank='Источник: Spotlight 6 p.82. Функции: greeting/help → play/date → seats/choice/price → payment → confirmation/collection.'}
      if(out[0]){
        out[0].situation='Напиши короткий диалог бронирования на новый спектакль.';
        out[0].steps=['Задай play и date.','Укажи/спроси number of seats.','Добавь выбор места или цену.','Заверши вопросом/ответом об оплате.'];
        out[0].check=['Роли customer/receptionist не перепутаны','Есть play/date/seats','Есть price/choice или payment','Диалог логичен'];
      }
      if(out[1])out[1].situation='Сделай мини-комикс Theatre Booking: 6–8 реплик с реальными booking phrases страницы 82.';
      if(out[2])out[2].situation='Подготовь только реплики Customer по плану. На следующем уроке другой ученик сыграет Receptionist и задаст реальные уточняющие вопросы.';
      if(out[3]){
        out[3].situation='Самостоятельно подготовь две role cards для будущего information gap: Customer знает нужный play/date/number of seats; Receptionist — доступные seats/prices/payment. На уроке карточки будут использованы в паре.';
        out[3].steps=['Заполни Customer card.','Заполни Receptionist card другой информацией.','Не пиши полный диалог.','Сохрани короткий ключ ожидаемых данных.'];
      }
      if(out[4]){
        out[4].situation='Создай 6 пар карточек Customer ↔ Receptionist для matching game. Не нужно придумывать 6 полных диалогов.';
        out[4].steps=['Выбери 6 функций бронирования.','Для каждой напиши одну реплику Customer и логичный ответ/вопрос Receptionist.','Раздели на 12 карточек.','Проверь роли и однозначность пар.'];
        out[4].check=['Ровно 6 пар','Реплики соответствуют ролям','Используются реальные функции бронирования','Партнёр дома не нужен'];
      }
    }
  ...
  function install(){
    if(window.__KA_METHOD_2423)return true;
    ...
    window.KA_METHOD_V24={version:'24.2.3',productBare,grammarEn,canDo,success,recommendedDefaults};
    window.__KA_METHOD_2423=true;
