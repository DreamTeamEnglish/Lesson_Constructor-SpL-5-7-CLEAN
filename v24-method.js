// ============================================================
// CLEAN v24.2.1 · Methodical normalization layer for GOLD 6
...
(function(){
  ...
  function success(l){
    ...
    if(idOf(l)==='6c')return[
      ...
      'I can complete simple Present Simple sentences in the Robinson Crusoe game.',
  ...
  function tuneActivity(l,a,newCanDo){
    ...
    if(code==='P3-02'){
      if(idOf(l)==='2a'){
        ...
        out.teacher='Кратко систематизирует ordinal numbers, months/seasons, telling the time и on/in/at; затем запускает короткий birthday interview по образцу учебника.';
        out.students='Составляют 2 своих примера и задают партнёру When’s your birthday? / What time…?; фиксируют один ответ.';
        out.instruction=`Use on / in / at correctly. Ask your partner: When’s your birthday? Then make one sentence with a date and one with a time.`;
        out.example=`My birthday is on 5th May. The party is at half past five. My birthday is in spring.`;
        out.materials='Spotlight 6, p. 17; ordinal numbers; months/seasons; clock/time models; prepositions table.';
        ...
        out.criterion='Дата/время понятны; on/in/at выбраны по смыслу; ученик может спросить/ответить о дне рождения.';
      }else if(idOf(l)==='6c'){
        out.title='Robinson Crusoe: Present Simple в игре';
        out.teacher='Использует реальную Robinson Crusoe board game страницы 60. Просит восстановить формы глаголов в 3-м лице и быстро сверить их в игровой последовательности.';
        out.students='Заполняют пропуски (sit, play, not feel, cook, miss, grow, explore, not have, eat, go, be, wake, wash) нужной формой Present Simple; затем сравнивают ответы в паре.';
        out.instruction=`Complete the Robinson Crusoe game with the correct Present Simple forms. Then compare your answers with a partner and explain one -s / doesn’t form.`;
        ...
        out.challenge='После проверки назвать 3 действия Robinson Crusoe без чтения клетки.';
        out.criterion='Формы Present Simple соответствуют подлежащему he и смыслу клетки.';
      }else if(idOf(l)==='8e'){
        out.title='Собираем и разыгрываем бронирование';
        out.teacher='Моделирует сокращённый ход реального диалога: play/date → seats → choice/price → payment. После первого прохода убирает часть опор. В конце 1–2 минуты можно использовать для /əʊ/ vs /aʊ/ из страницы 82.';
        out.students='Разыгрывают короткий диалог с заменой спектакля/даты/мест, затем повторяют с меньшей опорой; различают 2–3 слова на /əʊ/ и /aʊ/.';
        out.instruction=`Use the booking phrases. Change the play, date and seats. Then repeat with fewer prompts. Finish with: How would you like to pay? / Can I pay by credit card?`;
        ...
        out.materials='Spotlight 6, p. 82, model dialogue + pronunciation box /əʊ/ vs /aʊ/.';
        ...
        out.criterion='Реплики соответствуют ролям; бронирование идёт в логичном порядке; ключевые детали названы.';
      }else if(idOf(l)==='4f'){
    ...
    if(code==='P4-01'){
      if(idOf(l)==='2a'){
        ...
        out.teacher='Раздаёт информационно разные карточки Customer / Receptionist: play/date/number of seats и доступные rows/prices/payment. Следит, чтобы ученики действительно спрашивали недостающую информацию, а не читали один общий сценарий.';
        out.students='Проводят бронирование, уточняют play/date/seats/choice/price/payment; затем меняются ролями с новыми данными.';
        ...
        out.example=`R: How can I help you? C: I’d like to book some theatre tickets, please. R: Which play would you like to see? C: “The Snow Queen”, on Saturday. R: How many seats would you like? C: Two, please. R: We have two seats near the centre at £20 each. C: Great. Can I pay by credit card?`;
        out.materials='две ролевые карточки с разной информацией; Spotlight 6, p.82; чек-лист booking steps.';
  ...
  function install(){
    if(window.__KA_METHOD_2421)return true;
    ...
    window.KA_METHOD_V24={version:'24.2.1',productBare,grammarEn,canDo,success,recommendedDefaults};
    window.__KA_METHOD_2421=true;
